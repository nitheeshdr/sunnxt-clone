...
import { getFreeCarouselInfo } from "@/lib/api";
...
export default async function FreePage() {
  ...
  try {
    const res = await getFreeCarouselInfo();
  ...
  return (
    <div className="bg-[#0f0f0f] min-h-screen">
      <div className="px-8 py-6 max-w-350 mx-auto">
        <h1 className="text-2xl font-bold text-white mb-1">Free</h1>
        <p className="text-gray-400 text-sm">Watch free movies &amp; shows — no subscription needed</p>
      </div>
      ...
      {sections.length === 0 && (
        <div className="flex items-center justify-center h-64 text-gray-500">No content available right now.</div>
      )}
    </div>
  );
}
